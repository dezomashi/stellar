...
export default function DropCard({ signer, onMintSuccess }) {
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      setError(e.code === "ACTION_REJECTED" ? "Transaction rejected" : "Minting failed");
    } finally {
      setIsMinting(false);
    }
  };
            ...
            <div className="space-y-3 p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
              <div className="flex justify-between items-end text-white">
                <p className="text-xl font-mono font-bold tracking-tighter">
                  {supply.current} <span className="text-gray-600 text-sm italic">/ {supply.max}</span>
                </p>
            ...
            <div className="space-y-4">
              ...
              {error && (
                <motion.div 
                  ...
                  className="flex items-center justify-center gap-2 text-red-400 text-[10px] font-bold uppercase tracking-widest"
                >
